...
t_ray	ray_normal(t_object const obj, t_ray const ray, double const dist)
{
	...
	t_ray = ray_transform(ray, object.pipe);
	normal.pos = mtx_add(t_ray.pos, mtx_scalar_mult(dist, t_ray.dir));
	normal.dir = new_vec(obj.quadric.mtx[0] * normal.pos.mtx[0]
			...
			+ obj.quadric.mtx[11]);
	normal.pos = mtx_mult(obj.pipe.t_pos, normal.pos);
	normal.dir = mtx_mult(obj.pipe.t_normal, normal.dir);
